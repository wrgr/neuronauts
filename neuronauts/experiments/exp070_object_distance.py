...
def run(ctx: Context) -> Outcome:
    ...
    note = (
        ...
        f"touch an atom with no endpoint row at all -- equal, by construction, "
        f"to the {allp['mst']['n_links_endpoint_unreachable']} "
        f"endpoint-unreachable links -- and separately "
        f"{allp['mst']['object_only_rerouted']} object-only + "
        f"{allp['mst']['endpoint_only']} endpoint-only = "
        f"{allp['mst']['links_rerouted_between_metrics']} links are re-routed "
        f"between atoms both metrics see. These two counts landing on the same "
        f"number is a coincidence, not one number -- so EXP-060B's panel was "
        f"scored against a partly different answer key. "
