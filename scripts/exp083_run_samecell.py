...
def main():
    ...
    for ci, A_id in enumerate(cells):
        ...
        for b in range(len(BIN_NAMES)):
            ...
            for u in picks.tolist():
                ...
                donor = None
                for _ in range(40):
                    B_id = A_id if SAME_CELL else int(rng.choice([c for c in cells if c != A_id]))
                    dc = donor_cable[B_id]
                    ok = np.flatnonzero(np.abs(dc - target) <= CABLE_TOL * target)
                    if SAME_CELL:
                        # never the true piece, a piece of it, a removed piece,
                        # or an ancestor whose subtree contains the base
                        ok = ok[~(mu[ok] | mz[ok] | anc[ok])]
                    if len(ok):
    ...
    np.savez_compressed(OUT / "features_samecell.npz", X=X, cols=np.array(L.ALL_COLS),
