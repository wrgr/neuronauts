"""
EXP-035: Restored High-Performance Dual-Engine Benchmark with Full Confusion Matrices.
Combines:
  1. True 3D Continuous-Discrete Transformer Pointer Attention (from EXP-027 / EXP-029).
  2. 3D Fast Marching Geodesic Tracer + Hermite Spline Flux Integrals.
  3. Santiago Ramon y Cajal Conservation Laws (Space, Time, Material).
  4. Segment-Level Synapse Typing for Observable Grammar State.
  5. Complete Pairwise Merge and Split Contingency Matrices (TP, FP, FN, TN).
  6. Empirical Biological Grammar Violation Tracking.
Evaluated on 150 real proofread Minnie65 neurons (1,575 cuts, 450 fragments).
"""
...
def run_exp035_benchmark():
    print("=" * 120)
    print("EXP-035: RESTORED HIGH-PERFORMANCE DUAL-ENGINE BENCHMARK (150 REAL MINNIE65 NEURONS)")
    print("=" * 120)
    ...
    dual_engine = BlindCajalGeodesicDualEngine(emb_dim=64, geo_weight=2.5, cajal_weight=1.5, seed=42)
    ...
    for obj_id, toks in test_cells.items():
        ...
        for l_idx in leaf_indices:
            ...
            res = dual_engine.predict_blind_infill(
    ...
    print("\n" + "=" * 120)
    print("EXACT MEASURED EXP-035 RESTORED SCORECARD (30 UNTOUCHED TEST NEURONS, 90 FRAGMENTS)")
    print("=" * 120)
    ...
    print(f"Total Inference Latency / Cut:             {t_dual_ms / max(1, total_masks):.2f} ms")
    print("-" * 120)
    ...
    print("-" * 120)
    print(f"{'Metric':<35} {'Baseline v117':<25} {'EXP-035 Restored Dual-Engine':<30}")
    print("-" * 120)
