...
BINS_UM = [1, 3, 10, 30, 100, 300, 1000, 1e9]   # unused here
BIN_NAMES = ["5-10%", "10-20%", "20-35%", "35-50%"]
FRAC_BINS = [0.05, 0.10, 0.20, 0.35, 0.50]
...
def main():
    ...
    for ci, A_id in enumerate(cells):
        ...
        for b in range(len(BIN_NAMES)):
            lo, hi = FRAC_BINS[b] * total, FRAC_BINS[b + 1] * total
    ...
    np.savez_compressed(OUT / "features_big.npz", X=X, cols=np.array(L.ALL_COLS),
