"""
EXP-037: Tree-Beam MCTS, Bidirectional Handshake & Synaptic Fingerprinting Benchmark (150 Real Minnie65 Neurons).
Combines:
  1. Symmetric Bidirectional Handshake Consensus: P_handshake(A, B) = sqrt(P(B|A, t_A) * P(A|B, -t_B))
  2. Bipartite Synaptic Partner Jaccard Overlap: J_syn(A, B)
  3. Mechanical Bending Energy Regularization: E_bend = int ||gamma''(s)||^2 ds
  4. Tree-Beam MCTS Assembler over SANTIAGO Syntactic Derivations
...
def run_exp037_benchmark():
    print("=" * 120)
    print("EXP-037: TREE-BEAM MCTS, BIDIRECTIONAL HANDSHAKE & SYNAPTIC FINGERPRINTING BENCHMARK (150 REAL CELLS)")
    print("=" * 120)
    ...
    for obj_id, toks in test_cells.items():
        ...
        for l_idx in leaf_indices:
            ...
            if res["accepted"] and res["predicted_id"] is not None:
                mcts_links.append((soma_tok["fragment_id"], res["predicted_id"]))
    ...
    print("\n" + "=" * 120)
    print("EXACT MEASURED EXP-037 MCTS & HANDSHAKE SCORECARD (30 UNTOUCHED TEST NEURONS, 90 FRAGMENTS)")
    print("=" * 120)
    ...
    print(f"  Merge TP: {mcts_conf['merge']['tp']:>6d} | Merge FP: {mcts_conf['merge']['fp']:>6d} | Merge FN: {mcts_conf['merge']['fn']:>6d} | Merge TN: {mcts_conf['merge']['tn']:>6d}")
    print(f"  Merge Precision: {mcts_conf['merge']['precision']:.4f} | Merge Recall: {mcts_conf['merge']['recall']:.4f} | Merge F1: {mcts_conf['merge']['f1']:.4f} | Accuracy: {mcts_conf['merge']['accuracy']:.4f}")
    print("-" * 120)
    print("PAIRWISE SPLIT CONFUSION MATRIX (Positive = Keep Split / Separated):")
    print(f"  Split TP: {mcts_conf['split']['tp']:>6d} | Split FP: {mcts_conf['split']['fp']:>6d} | Split FN: {mcts_conf['split']['fn']:>6d} | Split TN: {mcts_conf['split']['tn']:>6d}")
    print(f"  Split Precision: {mcts_conf['split']['precision']:.4f} | Split Recall: {mcts_conf['split']['recall']:.4f} | Split F1: {mcts_conf['split']['f1']:.4f} | Accuracy: {mcts_conf['split']['accuracy']:.4f}")
    print("-" * 120)
    ...
    print("-" * 120)
    print(f"{'Metric':<35} {'Baseline v117':<25} {'EXP-037 MCTS & Handshake':<30}")
    print("-" * 120)
    print(f"{'Pairwise Out-of-Sample ARI':<35} {base_conf['ari']:>20.4f}  {mcts_conf['ari']:>28.4f}")
    print(f"{'Pairwise Merge Precision (Bar 1)':<35} {base_conf['merge']['precision']:>20.4f}  {mcts_conf['merge']['precision']:>28.4f}")
    print(f"{'Pairwise Merge Recall (Bar 2)':<35} {base_conf['merge']['recall']:>20.4f}  {mcts_conf['merge']['recall']:>28.4f}")
    print(f"{'Path-Weighted Precision (path_P)':<35} {base_path['path_P']:>20.4f}  {mcts_path['path_P']:>28.4f}")
    print(f"{'Path-Weighted Recall (path_R)':<35} {base_path['path_R']:>20.4f}  {mcts_path['path_R']:>28.4f}")
    print(f"{'Expected Run Length (ERL, um)':<35} {base_path['erl_um']:>20.1f}  {mcts_path['erl_um']:>28.1f}")
    print(f"{'Line Graph Synapse Precision':<35} {base_lg.pre_only.precision:>20.4f}  {mcts_lg.pre_only.precision:>28.4f}")
    print(f"{'Line Graph Circuit Recall':<35} {base_lg.pre_only.recall:>20.4f}  {mcts_lg.pre_only.recall:>28.4f}")
    print(f"{'Line Graph F1 Score':<35} {base_lg.pre_only.f1:>20.4f}  {mcts_lg.pre_only.f1:>28.4f}")
    print(f"{'Recovered True Synaptic Edges':<35} {base_lg.pre_only.tp:>20d}  {mcts_lg.pre_only.tp:>28d}")
    print("=" * 120)


if __name__ == "__main__":
    run_exp037_benchmark()
