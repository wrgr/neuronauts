...
trained model (A/B/C/D, balanced post sampling). Single-side uses the true post-neuron;
dual-side predicts both endpoints. AND metric requires both the pre-cluster \emph{and}
post-cluster to match the same true circuit edge. Post F1 measures v117 dendritic
fragment partition quality (post-neuron identity).}
\label{tab:connectivity}
\small
\begin{tabular}{lccccccc}
\toprule
Location & single F1 & dual F1 & AND F1 & post F1 & both-sides & ARI \\
\midrule
...
T4 ($y\!\in\![1000,1070]$ µm, north) & 0.701 & 0.651 & 0.898 & 0.932 & 1221 & 0.349 \\
\bottomrule
\end{tabular}
\end{table}

The connectivity F1 tracks merge precision rather than ARI: the reconstructed circuit
preserves most true connections even where ARI is depressed. The
dual-side F1 is the most demanding metric---it compounds errors from two independent
partitions---yet measures the circuit a downstream analyst would actually obtain. By design
the metric is sensitive to false merges and lenient toward over-fragmentation; it should
therefore be read alongside ARI and merge recall.

The \textbf{AND metric} (circuit-edge F1) is strictest: a synapse pair is correctly
recovered only when \emph{both} its pre-cluster and post-cluster match the same true
$(A \to B)$ edge. Even at T4---where ARI = 0.349 and single-side F1 = 0.701---the AND
metric remains \textbf{0.898}. This resilience arises from \textbf{post-side stability}:
the v117 dendritic fragment partition (post F1 $= 0.932$--$0.974$ across all four
locations) anchors the post-side even as the pre-side degrades, preserving most true
circuit edges in the joint representation. The post-side fragment partition---using raw
v117 segment IDs as cluster labels, without any GNN inference---thus provides a
high-quality dendritic-identity signal that is nearly invariant to the pre-side training
difficulty.
