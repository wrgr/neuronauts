...
\textbf{Result.} On the A/B/C training graph: $T = 0.97$, $\text{ECE} = 0.141$.
$T \approx 1$ indicates the raw logits are already nearly calibrated---temperature scaling
provides only a small correction. The reliability diagram (Figure~\ref{fig:calibration})
shows the model tracks the diagonal within $\pm 0.08$ for confidence bins between 0.2 and
0.8, with some deviation in the tails where sample counts are small.
