...
\begin{table}[h]
\centering
\caption{Variance across test locations, same A/B/C-trained model, $b = -2.0$.}
\label{tab:variance}
\small
...
\midrule
T1 $x \in [1150, 1350]$~\textmu{}m (ref.) & 383   & 0.613 & 0.977 & 0.537 & 0.007 \\
T2 $x \in [550,  750]$~\textmu{}m (west)  & 280   & 0.877 & 0.972 & 0.820 & 0.012 \\
T3 $y \in [870, 940]$~\textmu{}m (south)  & 143   & 0.829 & 0.991 & 0.744 & 0.004 \\
T4 $y \in [1000, 1070]$~\textmu{}m (north)& 2,277 & 0.287 & 0.958 & 0.676 & 0.015 \\
\midrule
\textbf{Mean $\pm$ std} & --- & \textbf{0.65 $\pm$ 0.23} & \textbf{0.97 $\pm$ 0.01} & \textbf{0.69 $\pm$ 0.11} & --- \\
\bottomrule
\end{tabular}
\end{table}

Merge precision is stable across all locations (range 0.958--0.991, std = 0.01). ARI
varies more (range 0.287--0.877), with T4 as the outlier: T4 is an order of magnitude
denser than the other test bboxes (2,277 vs.\ 143--383 synapses), has 33 frankenmerges
(vs.\ 1--7 for T1--T3), and lies entirely beyond the training bboxes in the $y$ direction.
Excluding T4, the three comparable-density bboxes give mean ARI = 0.77 $\pm$ 0.14,
merge\_P = 0.98 $\pm$ 0.01. Stable merge precision across all conditions is the
operationally relevant finding.

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{figures/variance_barchart}
\caption{ARI and merge\_P across the four in-column test locations (T1--T4).
Dashed lines indicate the cross-location mean. Merge precision is stable ($\sigma = 0.01$);
ARI varies, with T4 the outlier (6$\times$ denser, more frankenmerges).}
\label{fig:variance}
\end{figure}
...
\begin{table}[h]
\centering
\caption{Connectivity F1 across the four in-column test bboxes, $b = -2.0$. Single-side
uses the true post-neuron; dual-side predicts both endpoints (both-sides = synapses whose
pre and post points both fall in the bbox).}
...
false merges (merge\_P $= 0.97 \pm 0.01$, Section~\ref{sec:spatial}), the reconstructed
...
T4 illustrates the compounding effect: its single-side F1 (0.943) is only modestly below
T1--T3 because the true post-neuron absorbs dendritic errors. In dual-side mode (F1 $= 0.550$)
both pre- and post-partitions must be correct; the dense, out-of-training north bbox degrades
both simultaneously, exposing the harder regime.
