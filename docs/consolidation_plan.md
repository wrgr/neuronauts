...
| 2026-09-02 | 4 | **§4.2 DOCS ONLY threads moved.** Confirmed each of `experiments/{error_correction,grammar,cell_assignment,tree_dna,topology}/` held exactly one file, then `git mv` of each `README.md` to `docs/threads/{error_correction,grammar,cell_assignment,tree_dna,topology}.md`; the five now-empty dirs removed. Sibling links between the five (e.g. `../tree_dna/README.md`) rewritten to flat `docs/threads/` paths; the one link to a still-experiment-resident thread (`fingerprints`, not in this batch) rewritten to `../../experiments/fingerprints/README.md`. Every other repo reference to the old paths updated: `experiments/README.md`'s index table, `experiments/{root_neighborhood,pcfg,fingerprints,soma_graph}/README.md`, and `docs/archive/2026-09/README.md`'s own migration note. `grep -rn "experiments/<name>"` across `*.md`/`*.py`/`*.toml`/`*.cfg` came back clean afterward; no `.py` file referenced these dirs, so nothing was skipped. `tests/test_tracker_and_registry.py tests/test_experiment_runner.py` still **39 passed**. |
